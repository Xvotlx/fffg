...
async def play(client, m: Message):
    ...
    if replied:
        if replied.audio or replied.voice:
            await m.delete()
            huehue = await replied.reply("**🔄 جاري التشغيل انتظر قليلا**")
            ...
            if chat_id in QUEUE:
                ...
                await m.reply_photo(
                    photo="https://telegra.ph/file/780f3dc08fe392dcc145c.jpg",
                    caption=f"""
**🏷️ العنوان : [{songname}]({link})
💬 ايدي الدردشة : {chat_id}
🎧 طلب من : {m.from_user.mention}
💻 قناة السورس : [ قناة السورس ](t.me/XXi5X)**
""",
                )
            else:
...
💻 قناة السورس : [ سورس العمدة ](t.me/XXi5X)**
""",                  
         )
            await hmm.delete()
            await m.reply(  
